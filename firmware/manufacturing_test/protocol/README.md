...
Milestone 7 adds `json_protocol`, which uses the vendored MIT-licensed [jsmn](https://github.com/zserge/jsmn) tokenizer with a fixed 16-token buffer and no dynamic allocation. It accepts only the exact protocol-version-1 `START_TEST` request shape, builds a bounded metadata response, and produces structured errors for malformed or unsupported requests. USB callbacks still only move bytes; application-loop code reads completed lines, parses JSON, and queues response lines.
