...
The CLI loads configuration and reuses shared services to build current manufacturing firmware, discover one ST-Link, program and verify the ELF through STM32CubeProgrammer, and reset the target. External commands are never passed through a shell. The firmware now initializes USB CDC, handles OTG FS interrupts, and exposes bounded newline-framed transport; the computer-side protocol, reporting, and component-test Python modules remain responsibility markers for later milestones. Hardware-free tests cover external-command/programmer boundaries and the pure C newline framer, while real SWD and USB success are not claimed without a board.
