...
The package will eventually own configuration loading, tooling checks, firmware flashing, USB protocol communication, operator interaction, test orchestration, and result persistence. Milestone 2 provides only the initial `run --config` command, path validation, and startup summary; it does not interpret configuration contents or access hardware.
